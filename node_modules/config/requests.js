...
//Function for login  -----> chat menu
//parameter ID, PW
//after read on DB, reponse chat room imformation
exports.login = function(ID, PW, callback){
	var que = "SELECT * FROM tmp_client WHERE id ='" + ID+"' and PW='"+PW+"'";
	var query = connection.query(que, function(err, rows){
		if(rows.length == 0){
			callback({'result':"N",'type':"login"});
		}	
		else{
			//check chat room
			var que_for_chat = "SELECT no, partner,time_start FROM counseling_log WHERE client='"+ID+"'";
			var query_for_chat = connection.query(que_for_chat, function(err, rows){
				if(rows.length == 0){
					callback({'result':"Y",'type':"login"});
				}	
				else{
					var no_log = rows[0].no;
					var partner = rows[0].partner;
					var time_start = rows[0].time_start;
...
exports.send = function(no_log, from_id, msg, to_id, time_msg, device, callback){
  	...
  	var query = connection.query(que, function(err, rows){
		if(rows.length == 0){
      			callback({'response':"Failure",'type':"chat"});
    		}
    		else{
			var token = rows[0].token;
			console.log("token:"+token);
      			var que_for_no = "SELECT no FROM log_counseling WHERE no_log="+no_log+" order by id desc limit 1";
			console.log(que_for_no);
      			var query_for_no = connection.query(que_for_no, function(err, rows){
      				//if no values... it means never chat before
        			if(rows.length == 0){
          				no = Number(1);
					//console.log("length=0 so no = "+no);
					var que_insert_lc = "INSERT INTO log_counseling(no_log, no, time_message, log, log_from, device) VALUES("+ no_log+", "+no+", '"+time_msg+"', '" + msg +"', '" + from_id +"', '" + device +"')";
		      			//console.log(que_insert_lc);
		      			var query_insert_lc = connection.query(que_insert_lc, function(err, rows){
		        			if(err){
		          				callback({'response':"Failure",'type':"chat"});
		        			}
		        			else{
							request(
					              ...
					              , function (error, response, body) {
					                callback({'response':"Success" + "/ " + msg,'type':"chat"});
					              }
					          )
		        			}
		      			});
					...
					var que_insert_lc = "INSERT INTO log_counseling(no_log, no, time_message, log, log_from, device) VALUES("+ no_log+", "+no+", '"+time_msg+"', '" + msg +"', '" + from_id +"', '" + device +"')";
		      			...
		      			var query_insert_lc = connection.query(que_insert_lc, function(err, rows){
		        			if(rows.length == 0){
		        				...
		        				//push for making chat room on partner's device
							request(
					              { 
					                method: 'POST', 
					                uri: 'https://gcm-http.googleapis.com/gcm/send',
...
exports.loadmsg = function(index, no_log, callback){
  	...
  	//query for newest DB
  	var que = "SELECT no, time_message, log_from, log FROM log_counseling WHERE no_log = " + no_log +" ORDER BY id desc limit " + index +", " + i_var;
  	...
  	var query = connection.query(que, function(err, rows){
		...
		//	callback({'msg1':});			var ary = JSON.stringify('msg;
...
exports.makechat = function(time_start, partner, client, callback){
  	...
  	var query =  connection.query(que, function(err, rows){
		if(err){
          		callback({'response':"Failure",'type':"makechat"});
		}
      		else{
	  		...
	  		var query = connection.query(que, function(err, rows){
            			if(rows.length == 0){
              				...
              				var query2= connection.query(que2, function(err, rows){
					              ...
					              , function (error, response, body) {
					                callback({'no_log':no_log, 'partner':partner,'type':"makechat"});
					              }
					          )
              					}
              				});
